...
/*   Updated: 2026/03/30 20:28:50 by matoledo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3D.h"

int	chosen_color(int wall)
{
	if (wall == 1)
		return (0xFF0000);
	if (wall == 2)
...
void	paint_img(int pixel_column, int pixel_start, int pixel_end, void *img, int wall)
{
	char	*image;
	char	*image_aux;
	int		bits_per_pixel;
	...
	image = mlx_get_data_addr(img, &bits_per_pixel, &line_size, &endian);
	i = 0;
	while (i < SCREEN_HEIGHT)
	{
		if (i >= pixel_start && i <= pixel_end)
		{
			//nos vamos desplazando por el array de caracteres que representa la imagen
			//moviendonos de 4 en 4 (los bytes que sirven para representar el color de cada)
			memory_space = i * line_size + pixel_column * 4;
			image_aux = image + memory_space;
			*(unsigned int *)image_aux = mlx_get_color_value(screen()->mlx, chosen_color(wall));
		}
		i++;
	}
	return;
...
int	cub3D_loop(char **worldMap)
{
	...
	while(1)
	{
		...
		while (i < SCREEN_WIDTH)
		{
			...
			int	pixel_start;
			int	pixel_end;
			pixel_start = SCREEN_HEIGHT / 2 - height / 2;
			if (pixel_start < 0)
				pixel_start = 0;
			pixel_end = SCREEN_HEIGHT / 2 + height / 2;
			if (pixel_end > SCREEN_HEIGHT - 1)
				pixel_end = SCREEN_HEIGHT - 1;
			...
			paint_img(i, pixel_start, pixel_end, screen()->img, worldMap[map_x][map_y] - '0');
			i++;
		}
